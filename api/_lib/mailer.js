...
// Sends one email. Returns { sent: true } | { skipped, reason } | { error }.
// Never throws — callers (agent tools, brief) can inline the result.
export async function sendMail({ to, subject, html, text }) {
  ...
  try {
    await transport.sendMail({ from, to: recipients, subject, html, text });
    return { sent: true };
  } catch (err) {
